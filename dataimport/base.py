...
def finddateGaps(siteid, instrumentid, valuetype, startdate=None, enddate=None):
    ...
    Finds gaps in with given params

    :param siteid: int
    :param instrumentid: int
    :param valuetype: int list
    :param startdate: datetime
    :param enddate: datetime
    :return:
    """
    session = db.Session()
    dss = session.query(db.Dataset).filter(db.Dataset._site == siteid,
    ...
    dss = dss.filter(db.Dataset._valuetype.in_(valuetype)).all()
